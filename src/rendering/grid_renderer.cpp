...
void GridRenderer::resize(int cols, int rows) {
    cols_ = std::max(1, cols);
    rows_ = std::max(1, rows);
}

void GridRenderer::draw(Canvas& canvas) const {
    ...
    int maxCol = std::min(cols_ - 1, static_cast<int>(std::ceil(botRightWorld.x - 0.5f)));
    int minRow = std::max(0, static_cast<int>(std::floor(topLeftWorld.y - 0.5f)));
    int maxRow = std::min(rows_ - 1, static_cast<int>(std::ceil(botRightWorld.y - 0.5f)));

    if (minCol > maxCol || minRow > maxRow) return;

    // ── Draw board background ─────────────────────────────────────────────────
    ImVec2 boardTL = canvas.worldToScreen(0.0f, 0.0f);
    ImVec2 boardBR = canvas.worldToScreen(static_cast<float>(cols_),
    ...
    for (int row = minRow; row <= maxRow; ++row) {
        for (int col = minCol; col <= maxCol; ++col) {
            ...
            if (drawBeads) {
                ...
                // Inner bead (empty = white)
                dl->AddCircleFilled(centre, beadRadiusPx, kEmptyFill, segments);
                // Small centre peg
                if (beadRadiusPx > 5.0f) {
